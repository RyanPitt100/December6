# walkforward_h1atr.py
"""
Multi-instrument walk-forward evaluation for Trend_EMA_Pullback_H1ATR strategy.

This variant uses H1 ATR for SL/TP sizing while keeping 15m entries.
The goal is to have wider stops suitable for swing trading.
...
from strategies import TrendEMAPullback_H1ATR
from backtest_framework import simulate_trades, _compute_metrics
...
STRATEGY_CLASS = TrendEMAPullback_H1ATR
...
def run_walkforward_for_instrument(instrument: str) -> Optional[Dict]:
    ...
    try:
        # 1) Load data (with H1 ATR included)
        df_all = build_multi_tf_frame(instrument, include_h1_atr=True)
        if df_all is None or df_all.empty:
            print(f"  [SKIP] No data for {instrument}")
            return None

        # Verify atr_h1 column exists
        if "atr_h1" not in df_all.columns:
            print(f"  [SKIP] No atr_h1 column for {instrument}")
            return None
        ...
        for i, w in enumerate(windows, 1):
            ...
            trades_train = simulate_trades(df_train, sig_train)
            ...
            trades_test = simulate_trades(df_test, sig_test)
...
def main():
    print(f"{'='*80}")
    print(f"Multi-Instrument Walk-Forward Analysis (H1 ATR)")
    ...
    all_results_path = f"{OUTPUT_DIR}walkforward_H1ATR_all_instruments_summary.csv"
    ...
    if not df_shortlist.empty:
        shortlist_path = f"{OUTPUT_DIR}walkforward_H1ATR_shortlist.csv"
        ...
        mc_list_path = f"{OUTPUT_DIR}montecarlo_H1ATR_instruments.txt"
    ...
    print(f"\n{'='*80}")
    print(f"Multi-Instrument Walk-Forward Complete (H1 ATR)")
    print(f"{'='*80}")
